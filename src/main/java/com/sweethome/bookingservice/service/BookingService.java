	...
	public BookingInfoEntity doPayment( PaymentDto paymentDetails) throws Exception {
		...
		try {
			...
			if (bookingInfoOptional.isPresent()) {
				...
				producer.send(new ProducerRecord<String, String>("booking-events", "message", message), new Callback() {
					@Override
					public void onCompletion(RecordMetadata metadata, Exception e) {
						if (e != null) {
							// If there's an exception, it means the send failed
							System.err.println("Send failed for record with key \"" + "message" + "\" and value \"" + message + "\"\n" + e.getMessage());
						} else {
							// If no exception, the send was successful
							System.out.println("Successfully sent record to topic " + metadata.topic() +
